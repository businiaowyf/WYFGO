...
import (
	...
	"database/sql"
	"github.com/BLHT/HT_GOGO/gotcp/examples/ht_user"
...
func main() {
	...
	for uid < 5000000 {
		// for uid < 50000 {
		// Execute the query
		rows, err := db.Query("SELECT USERID, EXPIRETIME FROM HT_PURCHASE_TRANSLATE WHERE USERID >" + strconv.Itoa(uid) + " limit 1000")
		if err != nil {
			infoLog.Println("mysql Query failed")
			panic(err.Error()) // proper error handling instead of panic in your app
		}

		var sUid, sExpireTime []byte
		for rows.Next() {
			// get RawBytes from data
			err = rows.Scan(&sUid, &sExpireTime)
			if err != nil {
			...
			iTS, err := strconv.Atoi(string(sExpireTime))
			if err != nil {
				infoLog.Printf("strconv atoi failed expire time=%v", sExpireTime)
				continue
			}
			infoLog.Printf("uid=%v expire=%v", iUid, iTS)

			uid = iUid // 更新已经获取过的uid
			...
			*reqBody.VipExpireTs = uint32(iTS)
			infoLog.Printf("uid=%v ts=%v", iUid, *reqBody.VipExpireTs)
			...
			infoLog.Printf("National=%v, NativeLang=%v LearnLang1=%v LearnLang2=%v LearnLang3=%v TeachLang2=%v TeachLang3=%v",
				...
				*reqBody.TeachLang2,
				*reqBody.TeachLang3)
			infoLog.Printf("Nickname=%v VipExpireTs=%v",
