...
import (
	...
	"github.com/BLHT/HT_GOGO/gotcp/tcpfw/include/ht_p2p"
...
func ProcData(c *gotcp.Conn, p gotcp.Packet, nano int64) bool {
	...
	var ret uint16
	if err != nil {
		attr := "Forward/packet_failed"
		libcomm.AttrAdd(attr, 1)
		infoLog.Println("forward packet failed err=", err)
		result = uint16(ht_p2p.RET_CODE_RET_INTERNAL_ERR)
		SendResp(c, head, result)
	} else {
		result = uint16(ht_p2p.RET_CODE_RET_SUCCESS)
		rspPacket, ok := rsp.(*common.HeadV3Packet)
		if !ok {
			attr := "Forward/convert_v3packet_fail"
			libcomm.AttrAdd(attr, 1)
			infoLog.Println("Convert to HeadV3Packet failed")
		...
		ret = rspHead.Ret
		SendRespWithPayLoad(c, rspHead, rspPacket.GetBody(), result)
...
func main() {
	...
	workerApi = common.NewP2PWorkerApi(workerIp, workerPort, &common.HeadV3Protocol{})
